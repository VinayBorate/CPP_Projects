...
void setup_logger(const std::string &logger_name, const std::string &file_name, const std::string &log_level_str) {
    ...
    // Map string log levels to spdlog log levels
    ...
    if (it != log_levels.end()) {
        spdlog::set_level(it->second); // Set log level to the provided log_level
        ...
        spdlog::set_level(spdlog::level::info); // Default to info if invalid log level string
